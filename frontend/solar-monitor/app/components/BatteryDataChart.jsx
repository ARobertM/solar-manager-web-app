...
const BatteryVoltageChart = () => {
  const [chartData, setChartData] = useState({
    ...
    datasets: [{
      data: [],
      color: (opacity = 1) => `rgba(4, 164, 255, ${opacity})`, // BLUE
      strokeWidth: 2
    }]
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get('https://awfully-correct-doe.ngrok-free.app/api/influxdata-battery');
        const data = res.data;
        ...
        if (data && Array.isArray(data) && data.length > 0) {
          ...
          setChartData({
            ...
            datasets: [{
              data: chartValues,
              color: (opacity = 1) => `rgba(4, 164, 255, ${opacity})`, 
              strokeWidth: 2
            }]
          });
          setLoading(false);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.chartTitle}>Battery Voltage for the Last 24 Hours</Text>
      {error ? (
